...
static struct i2c_handle *g_i2c_handles[MAX_I2C_PERIPHERALS];
static struct dma_handle g_rx_dma_handles[MAX_I2C_PERIPHERALS];
static struct dma_handle g_tx_dma_handles[MAX_I2C_PERIPHERALS];
static struct dma_transfer g_trnf_conf[MAX_I2C_PERIPHERALS] = { [0 ... MAX_I2C_PERIPHERALS -
																 1] = { .per_sz = DMA_SZ_8,
																		.per_incr = false,
																		.mem_sz = DMA_SZ_8,
																		.mem_incr = true,
...
static void i2c_gpio_init(struct i2c_conf *conf)
{
	struct gpio_conf gpio_conf =
		gpio_conf_af(conf->sda, conf->af, GPIO_TYPE_OPEN_DRAIN, GPIO_PULL_UP);
	gpio_init(&gpio_conf);
...
static void i2c_hw_init(struct i2c_conf *conf, struct i2c_handle *handle)
{
...
static void i2c_configure_reload(volatile uint32_t *reg, struct i2c_handle *handle)
{
...
static void i2c_prepare_transaction(struct i2c_handle *handle)
{
	...
	MODIFY_FIELD(handle->i2c->CR2, I2C_CR2_SADD_Msk, I2C_CR2_SADD_Pos + 1,
				 handle->addr); // left shift by one
...
void i2c_ev_isr(enum i2c_perip type)
{
	struct i2c_handle *handle = g_i2c_handles[type];
	...
	{
		CLEAR_FIELD(handle->i2c->CR1, I2C_CR1_TXIE_Msk | I2C_CR1_RXIE_Msk | I2C_CR1_TXDMAEN_Msk |
		...
		if (handle->dma_mode) {
			g_trnf_conf[handle->perip].data_count = 0;
			i2c_start_dma(handle);
			uint32_t dma_msk = (handle->type == I2C_TYPE_RX) ? I2C_CR1_RXDMAEN_Msk :
															   I2C_CR1_TXDMAEN_Msk;
...
void i2c_er_isr(enum i2c_perip type)
{
	struct i2c_handle *handle = g_i2c_handles[type];
