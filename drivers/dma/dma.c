...
void dma_init(dma_conf_t *conf, dma_handle_t *handle)
{
    ...
    BW_ASSERT(conf->dmamux.num_req < 255, "Invalid DMAMUX requests: %d (Expected 1-254)", conf->dmamux.num_req);

    // Initialize clock
    dma_clock_init(conf->dma);

    DMA_Channel_TypeDef *dma_ch = (DMA_Channel_TypeDef *)((uint32_t)conf->dma + DMA_CHANNEL_OFFSET(conf->ch_no));
