...
static struct dma_handle *g_dma1_handles[6];
static struct dma_handle *g_dma2_handles[6];
...
void dma_init(struct dma_conf *conf, struct dma_handle *handle)
{
	...
	MODIFY_FIELD(dmamux->CCR, DMAMUX_CxCR_DMAREQ_ID_Msk, DMAMUX_CxCR_DMAREQ_ID_Pos,
				 conf->dmamux.dmareq_id);

	if (conf->dmamux.sync_en) {
		MODIFY_FIELD(dmamux->CCR, DMAMUX_CxCR_SYNC_ID_Msk, DMAMUX_CxCR_SYNC_ID_Pos,
					 conf->dmamux.sync_id);
		MODIFY_FIELD(dmamux->CCR, DMAMUX_CxCR_SPOL_Msk, DMAMUX_CxCR_SPOL_Pos,
					 conf->dmamux.sync_pol);
		MODIFY_FIELD(dmamux->CCR, DMAMUX_CxCR_NBREQ_Msk, DMAMUX_CxCR_NBREQ_Pos,
					 conf->dmamux.num_req + 1);
...
void dma_isr(DMA_TypeDef *dma, uint8_t ch_no)
{
	struct dma_handle *handle;
	if (dma == DMA1)
