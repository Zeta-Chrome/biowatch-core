...
{
    reg_set_mask(&RCC->AHB3ENR, RCC_AHB3ENR_IPCCEN_Msk);
    reg_set_mask(&RCC->AHB3ENR, RCC_C2AHB3ENR_IPCCEN_Msk);
...
bool hal_ipcc_is_tx_channel_occupied(uint8_t channel)
{
    ...
    return reg_get_bit(&IPCC->C1TOC2SR, IPCC_C1TOC2SR_CH1F_Msk + channel - 1);
}
...
{
    BW_ASSERT(channel <= 6 && channel >= 1, "Channel out of range %d (Expected 1-6)", channel);
    return reg_get_bit(&IPCC->C2TOC1SR, IPCC_C2TOC1SR_CH1F_Msk + channel - 1);
}
...
void hal_ipcc_deinit()
{
    ...
    reg_clear_mask(&RCC->AHB3ENR, RCC_AHB3ENR_IPCCEN_Msk);
}
