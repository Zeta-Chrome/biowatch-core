...
bw_status_t kernel_mem_init(const kernel_pool_conf_t *confs)
{
    uint32_t pool_idx = 0;
    for (int i = 0; confs[i].count > 0; i++)
    {
        ...
        for (int b = 0; b < count; b++)
        {
            if (b >= count - 1)
            {
                *(uint32_t *)(g_stack_pool + pool_idx + sz) = 0;
            }
            else
            {
...
bw_status_t kernel_mem_alloc(task_stack_t *stack)
{
    uint8_t block = mem_block_sz(stack->size);
    if (block >= MEM_BLOCK_COUNT || g_free_block_heads[block] == 0)
    {
        return STATUS_MEM_EXHAUSTED;
    }
...
bw_status_t kernel_check_mem_sanity(task_stack_t *stack)
{
...
void kernel_mem_dealloc(task_stack_t *stack)
{
