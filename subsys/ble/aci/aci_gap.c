                                              ...
                                              uint16_t conn_interval_max)
{
    ...
    index_input += 1;
    {
        ...
        index_input += 1;
        memcpy((void *)&cp1->service_uuid_list, (const void *)service_uuid_list, service_uuid_length);
                                                          ...
                                                          const peer_entry_t *peer_entry)
{
    ...
    index_input += 1;
    memcpy((void *)&cp0->peer_entry, (const void *)peer_entry, num_of_peer_entries * (sizeof(peer_entry_t)));
...
ble_status_t aci_gap_get_bonded_devices(uint8_t *num_of_addresses, bonded_device_entry_t *bonded_device_entry)
{
    hci_request_t rq;
...
aci_gap_add_devices_to_list(uint8_t num_of_list_entries, const list_entry_t *list_entry, uint8_t mode)
{
    ...
    aci_gap_add_devices_to_list_cp1 *cp1 =
        (aci_gap_add_devices_to_list_cp1 *)(cmd_buffer + 1 + num_of_list_entries * (sizeof(list_entry_t)));
    ble_status_t status = 0;
    int index_input = 0;
