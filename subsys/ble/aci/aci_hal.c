...
enum ble_status aci_hal_write_config_data(uint8_t offset, uint8_t length, const uint8_t *value)
{
...
enum ble_status aci_hal_read_config_data(uint8_t offset, uint8_t *data_length, uint8_t *data)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_set_tx_power_level(uint8_t en_high_power, uint8_t pa_level)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_le_tx_test_packet_number(uint32_t *number_of_packets)
{
	struct hci_request rq;
...
enum ble_status aci_hal_tone_start(uint8_t rf_channel, uint8_t freq_offset)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_tone_stop(void)
{
	struct hci_request rq;
	enum ble_status status = 0;
...
enum ble_status aci_hal_get_link_status(uint8_t *link_status, uint16_t *link_connection_handle)
{
	struct hci_request rq;
...
enum ble_status aci_hal_set_radio_activity_mask(uint16_t radio_activity_mask)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_get_anchor_period(uint32_t *anchor_period, uint32_t *max_free_slot)
{
	struct hci_request rq;
...
enum ble_status aci_hal_set_event_mask(uint32_t event_mask)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_set_peripheral_latency(uint8_t enable)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_read_rssi(uint8_t *rssi)
{
	struct hci_request rq;
...
enum ble_status aci_hal_ead_encrypt_decrypt(uint8_t mode, const uint8_t *key, const uint8_t *iv,
...
enum ble_status aci_hal_read_radio_reg(uint8_t register_address, uint8_t *reg_val)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_write_radio_reg(uint8_t register_address, uint8_t register_value)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_read_raw_rssi(uint8_t *value)
{
	struct hci_request rq;
...
enum ble_status aci_hal_rx_start(uint8_t rf_channel)
{
	struct hci_request rq;
	uint8_t cmd_buffer[HCI_COMMAND_MAX_PARAM_LEN];
...
enum ble_status aci_hal_rx_stop(void)
{
	struct hci_request rq;
	enum ble_status status = 0;
