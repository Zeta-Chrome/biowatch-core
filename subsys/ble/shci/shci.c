...
bw_status_t shci_send(uint16_t cmd_code, uint8_t cmd_pl_len, uint8_t *p_cmd_pl, tl_evt_packet_t *p_rsp)
{
    ...
    tl_sys_send_cmd();
    bw_status_t status = kernel_semaphore_take(&g_cmd_rx_sem, 1000);
...
shci_cmd_status_t shci_c2_debug_init(shci_c2_debug_init_cmd_packet_t *p_cmd_packet)
{
    ...
    shci_send(SHCI_OPCODE_C2_DEBUG_INIT, sizeof(shci_c2_debug_init_cmd_param_t), (uint8_t *)&p_cmd_packet->param, p_rsp);
...
shci_cmd_status_t shci_c2_ble_init(shci_c2_ble_init_cmd_packet_t *p_cmd_packet)
{
    ...
    shci_send(SHCI_OPCODE_C2_BLE_INIT, sizeof(shci_c2_ble_init_cmd_param_t), (uint8_t *)&p_cmd_packet->param, p_rsp);
...
shci_cmd_status_t shci_get_wireless_fw_info(wireless_fw_info_t *p_wireless_info)
{
     ...
     *  This table is stored in RAM at startup during the TL (transport layer) initialization
     */
    p_wireless_info->version_major = ((wireless_firmware_version & INFO_VERSION_MAJOR_MASK) >> INFO_VERSION_MAJOR_OFFSET);
    p_wireless_info->version_minor = ((wireless_firmware_version & INFO_VERSION_MINOR_MASK) >> INFO_VERSION_MINOR_OFFSET);
    p_wireless_info->version_sub = ((wireless_firmware_version & INFO_VERSION_SUB_MASK) >> INFO_VERSION_SUB_OFFSET);
    p_wireless_info->version_branch = ((wireless_firmware_version & INFO_VERSION_BRANCH_MASK) >> INFO_VERSION_BRANCH_OFFSET);
    p_wireless_info->version_release_type = ((wireless_firmware_version & INFO_VERSION_TYPE_MASK)
                                             >> INFO_VERSION_TYPE_OFFSET);

    p_wireless_info->memory_size_sram2b = ((wireless_firmware_memorySize & INFO_SIZE_SRAM2B_MASK)
                                           >> INFO_SIZE_SRAM2B_OFFSET);
    p_wireless_info->memory_size_sram2a = ((wireless_firmware_memorySize & INFO_SIZE_SRAM2A_MASK)
                                           >> INFO_SIZE_SRAM2A_OFFSET);
    p_wireless_info->memory_size_sram1 = ((wireless_firmware_memorySize & INFO_SIZE_SRAM1_MASK) >> INFO_SIZE_SRAM1_OFFSET);
    p_wireless_info->memory_size_flash = ((wireless_firmware_memorySize & INFO_SIZE_FLASH_MASK) >> INFO_SIZE_FLASH_OFFSET);

    p_wireless_info->stack_type = ((wireless_firmware_infoStack & INFO_STACK_TYPE_MASK) >> INFO_STACK_TYPE_OFFSET);

    /**
     *  Retrieve the FusInfoTable
     *  This table is stored in RAM at startup during the TL (transport layer) initialization
     */
    p_wireless_info->fus_version_major = ((fus_version & INFO_VERSION_MAJOR_MASK) >> INFO_VERSION_MAJOR_OFFSET);
    p_wireless_info->fus_version_minor = ((fus_version & INFO_VERSION_MINOR_MASK) >> INFO_VERSION_MINOR_OFFSET);
    p_wireless_info->fus_version_sub = ((fus_version & INFO_VERSION_SUB_MASK) >> INFO_VERSION_SUB_OFFSET);

    p_wireless_info->fus_memory_size_sram2b = ((fus_memorySize & INFO_SIZE_SRAM2B_MASK) >> INFO_SIZE_SRAM2B_OFFSET);
    p_wireless_info->fus_memory_size_sram2a = ((fus_memorySize & INFO_SIZE_SRAM2A_MASK) >> INFO_SIZE_SRAM2A_OFFSET);
    p_wireless_info->fus_memory_size_flash = ((fus_memorySize & INFO_SIZE_FLASH_MASK) >> INFO_SIZE_FLASH_OFFSET);
