...
PLACE_IN_SECTION("MB_MEM2")
ALIGN(4) static shci_c2_debug_general_config_t g_debug_general_config = {0, 0, {0, 0}, 0, 0, 0, 0, 0};
...
static bool sys_ready_evt_handler(tl_async_evt_t *p_sys_evt)
{
    ...
    if (p_sys_ready_event->sysevt_ready_rsp == WIRELESS_FW_RUNNING)
    {
        ...
        shci_cmd_status_t status = shci_c2_debug_init(&dbg_cmd_pkt);
...
static void sys_err_evt_handler(tl_async_evt_t *p_sys_evt)
{
    ...
    p_sys_error_code = (shci_sys_err_code_t *)p_sys_evt->payload;
...
static void sys_evt_task(void *user_data)
{
    ...
    wireless_fw_info_t wireless_info;
                              ...
                              bool bonding_mode)
{
    ...
    ret = aci_hal_write_config_data(CONFIG_DATA_IR_OFFSET, CONFIG_DATA_IR_LEN, (uint8_t *)g_ble_ir_value);
    if (ret != BLE_STATUS_SUCCESS)
    {
    ...
    ret = aci_hal_write_config_data(CONFIG_DATA_ER_OFFSET, CONFIG_DATA_ER_LEN, (uint8_t *)g_ble_er_value);
    if (ret != BLE_STATUS_SUCCESS)
    {
    ...
    ret = aci_gatt_update_char_value(gap_service_handle, gap_appearance_char_handle, 0, 2, (uint8_t *)&appr);
    if (ret != BLE_STATUS_SUCCESS)
    {
              ...
              bool bonding_mode)
{
    ...
    shci_cmd_status_t status = shci_c2_ble_init(&ble_init_cmd_pkt);
    if (status != SHCI_SUCCESS)
    {
    ...
    ble_gap_gatt_init(name, appearance, io_capability, mitm_protection, secure_support, bonding_mode);
